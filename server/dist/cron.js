...
export function evaluateYesterdayResults() {
    ...
    const activeHabits = db.prepare("SELECT * FROM habits WHERE category = 'active' AND is_active = 1").all();
    const users = ['sereja', 'lera'];
    for (const userId of users) {
        const user = getUser(userId);
        if (!user)
            continue;
        // Check violations yesterday
        const violationsCount = db.prepare('SELECT COUNT(*) as c FROM violations WHERE user_id = ? AND date = ?').get(userId, yesterdayStr).c;
    ...
    broadcastTelegram(`⏰ *Итоги вчерашнего дня (${yesterdayStr}) подведены!*\n\n` +
...
async function sendEveningReminders() {
    ...
    `).get(user.id, todayStr).c;
        const remaining = activeHabits.length - completed;
        if (remaining > 0) {
            try {
                await broadcastTelegram(`🔔 *Напоминание на вечер для ${user.name}:*\n` +
